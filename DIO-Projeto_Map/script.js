// Atividade 1: Maps
// Crie uma função que retorna o nome dos membros de um Map que tem o papel 'ADMIN' no sistema.

// Crie uma função getAdmins que recebe um Map;
// Crie um Map e popule-o com nomes de usuários e seus papeis no sistema. (Ex: 'Luiz' => 'Admin');
// Dentro de getAdmins, utilize o loop for...of para retornar uma lista com os nomes dos usu;arios que são administradores.


function retornaNomes(admin){
    let ADMs = []
    for([key, value] of admin){
        if(value === 'Admin'){
            ADMs.push(key)
        }
    }
    return ADMs
}

const usuario = new Map()

usuario.set('Cleberton', 'Admin')
usuario.set('Lavínia', 'User')
usuario.set('Nicolas', 'User')
usuario.set('Izabel', 'Admin')

console.log(retornaNomes(usuario))